...
// *** begin
    ...
    å‹•ä½œãƒ–ãƒ©ã‚¦ã‚¶ :: IE4+ , NN4.06+ , Gecko , Opera6+
    ...
    *   Usage:
    ...
    strMode -> å…¥åŠ›ãƒ‡ãƒ¼ã‚¿ãŒæ–‡å­—åˆ—ã®å ´åˆ 1 ã‚’
...
// 
{
    ...
    var encoder = function(_ary)
    {
        ...
        if (md) // 3ã®å€æ•°ã«ãƒ‘ãƒ‡ã‚£ãƒ³ã‚°ã—ãŸ 0x0 åˆ† = ã«ç½®ãæ›ãˆ
        {
            md = 3- md;
    ...
    var decoder = function(_b64)
    {
        ...
        // replace æ™‚ = ã‚‚å‰Šã£ã¦ã„ã‚‹ã€‚ãã® = ã®ä»£ã‚ã‚Šã« 0x0 ã‚’è£œé–“
...
//    var unpackChar  = function(_x){ return window[utfLibName].unpackChar(_x) };
    ...
    * [RFC 1738] Uniform Resource Locators (URL)
    ...
    // æ–‡å­—åˆ—ã‚’ UTF16 (æ–‡å­—ã‚³ãƒ¼ãƒ‰) ã¸
    utf16code_array = utf.unpackUTF16( my_string );

    // æ–‡å­—åˆ—ã‚’ UTF8 (æ–‡å­—ã‚³ãƒ¼ãƒ‰) ã¸
    utf8code_array = utf.unpackUTF8( my_string );
    
    // UTF8 (æ–‡å­—ã‚³ãƒ¼ãƒ‰) ã‹ã‚‰æ–‡å­—åˆ—ã¸ã€‚ utf.unpackUTF8() ã—ãŸã‚‚ã®ã‚’å…ƒã«æˆ»ã™
    my_string = utf.packUTF8( utf8code_array );

    // UTF8/16 (æ–‡å­—ã‚³ãƒ¼ãƒ‰) ã‚’æ–‡å­—åˆ—ã¸
    my_string = utf.packChar( utfCode_array );
    
    // UTF16 (æ–‡å­—ã‚³ãƒ¼ãƒ‰) ã‹ã‚‰ UTF8 (æ–‡å­—ã‚³ãƒ¼ãƒ‰) ã¸
    utf8code_array = utf.toUTF8( utf16code_array );
    
    // UTF8 (æ–‡å­—ã‚³ãƒ¼ãƒ‰) ã‹ã‚‰ UTF16 (æ–‡å­—ã‚³ãƒ¼ãƒ‰) ã¸
    utf16code_array = utf.toUTF16( utf8code_array );



    // URL æ–‡å­—åˆ—ã¸ã‚¨ãƒ³ã‚³ãƒ¼ãƒ‰
